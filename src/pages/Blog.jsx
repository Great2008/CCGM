...
export default function Blog() {
  ...
  // force=true (pull-to-refresh) skips the daily snapshot and goes straight
  // to live Supabase — same behavior as Sermons/Sabbath School/Devotional.
  ...
  useEffect(() => {
    loadPosts().then(data => {
      setPosts(data)
      // Deep link: /blog/:id opens straight to that post. Otherwise default
      // to the first (most recent) post, same as before.
      const match = id ? data.find(p => String(p.id) === String(id)) : null
      setSelected(match || data[0] || null)
      setLoading(false)
    })
  }, [loadPosts]) // eslint-disable-line
  ...
  return (
    ...
    <div style={{ overflowX: 'hidden', width: '100%' }}>
      ...
      {/* Hero */}
      <div className="blog-hero" style={{ background: 'linear-gradient(135deg, var(--brand-deep) 0%, var(--brand-mid) 100%)', padding: 'clamp(90px,14vw,130px) 5% 56px', textAlign: 'center' }}>
